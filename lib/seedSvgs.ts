/**
 * SVG generators for the seeded demo triangles. Real posts carry uploaded
 * photos; these give the demo content something to show. Written to the
 * uploads directory as .svg files at seed time (see lib/db.ts).
 */
...
function sceneMarkup({ scene, accent }: SeedImageSpec): string {
  switch (scene) {
    case "mountain":
      return `
    ...
    case "sign":
      return `
        ...
        <polygon points="270,160 530,160 400,390" fill="#ffffff" stroke="${accent}" stroke-width="40" stroke-linejoin="round"/>`;
    case "roof":
      return `
    ...
    case "sandwich":
      return `
        ...
        <polygon points="170,460 470,460 470,220" fill="#fcd34d" stroke="${accent}" stroke-width="14" stroke-linejoin="round"/>
        <polygon points="330,480 630,480 630,240" fill="#fbbf24" stroke="${accent}" stroke-width="14" stroke-linejoin="round"/>
    ...
    case "chip":
      return `
        ...
        <polygon points="400,150 645,480 155,480" fill="#f59e0b" stroke="${accent}" stroke-width="16" stroke-linejoin="round"/>
    ...
    case "stairs":
      return `
        ...
        <path d="M 170 490 L 285 490 L 285 405 L 400 405 L 400 320 L 515 320 L 515 235 L 630 235 L 630 150 L 630 490 Z" fill="${accent}"/>
        <line x1="170" y1="470" x2="610" y2="145" stroke="#ffffff" stroke-width="10" opacity="0.6"/>
        <rect y="490" width="800" height="110" fill="#000000" opacity="0.15"/>`;
  }
}

/** Full standalone SVG document for a seed image. */
export function seedSvg(spec: SeedImageSpec): string {
